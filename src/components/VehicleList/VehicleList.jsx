...
const VehicleList = () => {
  ...
  const filteredVehicles = vehicles.filter(vehicle => Array.isArray(vehicle.location) && vehicle.location.includes("KJC KOTHANUR"));

  const quantity = filteredVehicles.map(vehicle =>
    vehicle.location.findIndex(location => location === "KJC KOTHANUR") !== -1 ?
      vehicle.quantity[vehicle.location.findIndex(location => location === "KJC KOTHANUR")] :
      ""
  );
